...
def send_job_listings_to_codeforcash(listings):
    for listing in range(len(listings)):
        ...
        try:
            ...
            # with open('pre_BadWords_Check_Descriptions.txt','a+') as f:
            #     f.write(json.dumps(web_data))


            #move to outer loop above.. break or continue?
            ...
            if bad_word_count == 0:
                no_bad_words = data_to_send_in_request_body
            else:
                continue
        ...
        # with open('pre_Post_Listings.txt','a+') as f:
