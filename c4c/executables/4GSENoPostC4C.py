...
def pass_different_clients():
    for client in clients:
        ...
        do_google_search(cse_search_term)

def do_google_search(search_term, api_key=API_KEY_TO_USE_FOR_THIS_RUN, cse_id=CSE_ID_TO_USE_FOR_THIS_RUN, start=CSE_INDEX_START, **kwargs):
...
def save_gse_call_results(listings):
    with open('finalResults.txt','a+') as f:
        f.write(json.dumps(pass_different_clients(), sort_keys = True,
                indent = 4))
