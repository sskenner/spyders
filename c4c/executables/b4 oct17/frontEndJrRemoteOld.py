...
def get_job_listings_from_google():
    results = []
    for number_of_search_result_being_processed in range(1,2):
        results.append(do_google_search(
            ...
            num=1, start=number_of_search_result_being_processed))
    return results
...
def send_job_listings_to_codeforcash(listings):
    for index, listing in enumerate(listings):
        data_to_send_in_request_body = {
            ...
            'website': listings[index]['link'],
            'description': listings[index]['snippet'],
